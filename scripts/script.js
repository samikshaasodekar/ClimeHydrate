const apiKey = 'f216c49b84ff16ec867822231d69f8af'; // Replace with your valid API key
...
function calculateRecommendedIntake(weight, exerciseMinutes, temperature) {
    let weightInPounds = weight * 2.20462; // Convert kg to pounds
    let recommendedIntakeOunces = weightInPounds * 0.5; // Half an ounce per pound of body weight
    recommendedIntakeOunces += (exerciseMinutes / 30) * 12; // 12 ounces for every 30 minutes of exercise
    if (temperature >= 20) {
        recommendedIntakeOunces += 16; // Add 16 ounces if the temperature is 20°C or higher
    }
...
function updateWaterRecommendation() {
    ...
    const maxWeight = 635; // Maximum reasonable weight in kg
    const maxExercise = 1440; // Maximum exercise duration in minutes
